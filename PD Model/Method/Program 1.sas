 ...
 proc import datafile="/folders/myfolders/Truist-Credit_Risk_SAS/PD Model/Data/test_del_dataset.csv"
      dbms=csv
      out=PD_DATA.test_final_del
      replace;    
 run;
...
/* data PD_DATA.train_Final_del; */
/* set PD_DATA.train_Final_del; */
/* if act_upb  = . then act_upb = Orig_amt ; */
/* UPB_Ratio = act_upb / orig_amt; */
/* run; */
...
%let MacroTransf = QGT_GDP QRT_GDP AG_GDP QDT_UMP QGT_UMP AG_UMP ggr lagging_UMP GDP_Annual_Rate QDT_GDP;

%let CUR_var = CLTV dti cscore_b orig_amt purpose curr_rte loan_age Orig_chn Orig_rt Orig_trm Num_bo Orig_dte &Macros &MacroTransf diff_mortgage;

%let DEL_var = CLTV curr_rte dti cscore_b GDP HS QDT_UMP ratio_upb GDP_Annual_rate ratio_upb Mortgage30US QDT_GDP;
...
ods rtf file="&p_report/Current_Model.rtf"  style=Sapphire bodytitle nogtitle;
ods rtf select all;            
...
ods graphics / noborder height=5.3333in width=8in MAXOBS=100000000;
...
/* Sub-Prime */
%let c_Var = Purpose Orig_chn ;
%let MacroTransf = GDP_annual_rate  ;
%let CUR_Var = CLTV Dti diff_Mortgage cscore_b &MacroTransf  ; 
%test(1);
...
%let Macros = Permits  ;
%let MacroTransf = GDP_annual_rate  ;
%let CUR_Var = CLTV Dti diff_Mortgage cscore_b  &Macros &MacroTransf  ; 
...
%let d_pd = DEL;
%let c_var = ;
%let DEL_Var = CLTV curr_rte cscore_b HS QDT_UMP;
...
%let d_pd = DEL;
%let c_var = ;
%let DEL_Var = CLTV cscore_b GDP_Annual_Rate ;
%test(1); 
...
/* data xyz ; */
/* set PD_DATA.train_final_cur; */
/* if next_stat = 'DEL' then Target_Var = 1; */
/* else target_var = 0; */
/* run; */
/*  */
/* proc reg data=xyz; */
/* model  target_var = CLTV Dti Cscore_b diff_mortgage  Permits GDP_annual_rate QDT_UMP  / vif tol collin; */
/* run; */
